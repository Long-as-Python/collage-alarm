...
Write-Host "`n1. Checking if port 4000 is listening..."
...
if ($port) {
    ...
    Write-Host "   ✗ Port 4000 is NOT listening - Backend may not be running!" -ForegroundColor Red
}

# Try to connect to the backend
Write-Host "`n2. Attempting to connect to http://localhost:4000/api/auth/verify..."
try {
    $response = Invoke-WebRequest -Uri "http://localhost:4000/api/auth/verify" -Method GET -TimeoutSec 3 -ErrorAction Stop
...
try {
    ...
    $response = Invoke-WebRequest -Uri "http://localhost:4000/api/auth/admin-login" `
        -Method POST `
...
Write-Host "If port 4000 is NOT listening:" -ForegroundColor Yellow
...
Write-Host "  powershell -Command `"Stop-Process -Force -ErrorAction SilentlyContinue -Id (Get-NetTCPConnection -LocalPort 4000).OwningProcess`""
